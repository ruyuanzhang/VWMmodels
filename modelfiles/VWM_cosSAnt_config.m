function c = VWM_cosSAnt_config(varparams)
...
opt.PLB = [0, 0, 0, 0, 0];
opt.PUB = [maxJ1bar, 20, maxJ1bar, 2*pi, 1];
opt.LB = [0, 0, 0, 0, 0];
opt.UB = [maxJ1bar, 20, maxJ1bar, 2*pi, 1];
opt.paramLabels = {'Jm', 'K', 'Jf', 'muf', 'pNT'};
...
c.fitFun = @VWM_cosSAnt_fit;

%% define negative loglikelihood function
c.negLogLikeliFun=@VWM_cosSAnt_nll;
