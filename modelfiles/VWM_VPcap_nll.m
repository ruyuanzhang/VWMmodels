function LLH = VWM_VPcap_nll(pars, data)
...
for ii=1:length(data.unique_N)
    ...
    % p_error = p_error/sum(p_error) * 1/diff(data.gvar.error_range(1:2));
    % If use PROBABILITY
    ...
    if data.unique_N(ii) > K % N iterm exceeds capacity limits 
        p_resp = K/data.unique_N(ii) * p_resp + (1-K/data.unique_N(ii)) * 0.5/90;  
    end
