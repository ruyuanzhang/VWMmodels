function [fitpars, neglh, neglhtrial, AIC, AICc, BIC] = fit_VPcapSwap_model(N, probe, nt, resp, x0, opt)
...
gvar.n_par          = 6;                     % number of parameters (J1bar, power, tau, kappa_r, K, s)
...
[x,fval,exitflag,output,optimState,gpstruct] = bads(@(params) compute_LLH_VPcapSwap(params, data, gvar),x0,LB,UB,PLB,PUB);
...
fitpars = x;
% compute AIC and BIC
n_free_pars = gvar.n_par - (numel(unique_N)==1);   % Jbar, tau, power, kappa_r  (4 if we have only 1 set size)
