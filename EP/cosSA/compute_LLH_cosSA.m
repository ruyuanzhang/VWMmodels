...
for ii=1:length(N_vec)
    ...
    if N <= K % memory load within capacity with probability K/N
        ...
        for jj=1:length(gvar.error_range)
                ...
                p_error(jj,:) = p2 * 1/(2*pi)/besseli0_fast(kappa2,1).*exp(kappa2.*cosd(mu)) + ...
                    p1 * 1/(2*pi)/besseli0_fast(kappa1,1).*exp(kappa1.*cosd(mu));
        end
        ...
        for jj=1:length(gvar.error_range) % loop error
                mu=gvar.error_range(jj)-bias;
                p_error(jj,:) = (K/N)*1/(2*pi) / besseli0_fast(kappa1, 1).*exp(kappa1*cosd(mu)) + ...
                (1-K/N)*(0.5/90);
        end
